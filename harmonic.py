...
parser.add_option("-v", "--verbose", dest="verbose", action="store_true",default=False,
                  help="more verbosity")
parser.add_option("-N", "--number of samples", nargs = 2, dest="N_k", type="int",default=[100000,100000],
                  help="number of samples from the two states, [default = %default]") 
parser.add_option("-D", "--number of dimensions", dest="D", type="int",default=1,
...
#   V(x;K) = \sum_{i=1}^D (K/2) * (x_i-x_0,i)**2                                                                             
...
#   f(beta,K) = - (D/2) * ln[ (2 pi) / (beta K) ]              
#   f_2 - f_1 = -(D/2) (ln [ 1/ beta_2 K_2] - ln [ 1/beta_1 K_1])  
#   f_2 - f_1 = (D/2) ln [beta_2/K_2 / beta_1/K_1]  
#   f_2 - f_1 = (D/2) ln [K_1 T_1/ K_2 T_2]  
#   for T_1 = 0.9, T_2 = 1.1, K_1 = K_2, df = 0.5*ln(1.1/0.9) = 0.10034D
#   for T_1 = 0.8, T_2 = 1.2, K_1 = K_2, df = 0.5*ln(1.2/0.8) = 0.2027D
#   for T_1 = 0.5, T_2 = 1.5, K_1 = K_2, df = 0.5*ln(1.5/0.5) = 0.5493D
#   analytical variance = depends on the method.  So won't try 
#
#   average energy is -\pd{ln Q}{beta}
#   ln Q  = (D/2) * ln[ (2 pi) / (beta K) ]             
#   -pd{-(D/2) * ln[ (2 pi) / (beta K)}{\beta} = 
#   D/2 pd{ln[(beta K)}{\beta} = 
#   D/2 pd{ln[(beta)}{\beta} = 
#   D/2 beta^{-1} 
#
...
for n in range(ngen):
    ...
    for k in range(K):
        ...
        if (noise > 0):
            U_kn[k,:] += D*numpy.abs(noise*numpy.random.normal(O_k[k],1,[N_k[k]])) # add noise to the potential
